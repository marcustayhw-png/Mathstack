...
import {
  ArrowRight, BookOpen, ChevronLeft, Sprout, TrendingUp, GraduationCap,
  Loader2, FileText, ExternalLink, FlameKindling, PenTool,
...
// ── Satori brand colour palette for levels ──────────────────────────
const LEVELS = [
  {
    id: "sec1" as Level,
...
// Strand → icon + Satori palette colours
const STRAND_META: Record<string, { icon: React.ElementType; bg: string; text: string; border: string }> = {
  "Number & Algebra":      { icon: Hash,           bg: "bg-primary/8",                    text: "text-primary",                    border: "border-primary/20" },
  "Geometry & Measurement":{ icon: Compass,        bg: "bg-[oklch(0.82_0.08_55)/30]",     text: "text-[oklch(0.40_0.10_55)]",      border: "border-[oklch(0.62_0.12_55)/25]" },
  "Statistics & Probability":{ icon: BarChart3,    bg: "bg-[oklch(0.88_0.04_148)/40]",    text: "text-[oklch(0.35_0.09_148)]",     border: "border-[oklch(0.65_0.10_148)/25]" },
  "Algebra":               { icon: Calculator,     bg: "bg-primary/8",                    text: "text-primary",                    border: "border-primary/20" },
  "Geometry & Trigonometry":{ icon: Triangle,      bg: "bg-[oklch(0.82_0.08_55)/30]",     text: "text-[oklch(0.40_0.10_55)]",      border: "border-[oklch(0.62_0.12_55)/25]" },
  "Calculus":              { icon: FunctionSquare, bg: "bg-[oklch(0.88_0.04_148)/40]",    text: "text-[oklch(0.35_0.09_148)]",     border: "border-[oklch(0.65_0.10_148)/25]" },
};
const STRAND_FALLBACK = { icon: BookOpen, bg: "bg-muted/40", text: "text-muted-foreground", border: "border-border/40" };
...
function Breadcrumb({
  ...
  const steps: { key: Step; label: string; onClick?: () => void }[] = [
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {steps.map((s, i) => {
        if (s.key === "subject" && !subject && step !== "subject") return null;
...
export function NotesClient() {
  ...
  const fetchChapters = useCallback(async () => {
    ...
    if (selectedLevel === "sec34" && !subject) return;
  ...
  useEffect(() => { if (step === "chapter") fetchChapters(); }, [step, fetchChapters]);

  useEffect(() => {
    if (step !== "content" || !selectedChapter) return;
    setContentLoading(true);
    const p = new URLSearchParams({ chapter_id: selectedChapter.id });
    Promise.all([
      fetch(`/api/notes?${p}`).then(r => r.ok ? r.json() : []),
      fetch(`/api/practice?${p}`).then(r => r.ok ? r.json() : []),
    ]).then(([n, pr]) => { setNotes(n); setPracticeItems(pr); })
  ...
  return (
    <div className="relative w-full">
      ...
      {/* Breadcrumb — shown on all steps except the first */}
      <AnimatePresence>
        {step !== "level" && (
          <motion.div
          ...
          >
            <Breadcrumb
              step={step} level={selectedLevel} subject={selectedSubject}
              onLevel={handleBackToLevel} onSubject={handleBackToSubject}
            />
            <button
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "level" && (
          <motion.div key="level" {...popIn} className="space-y-10">
            <div className="mx-auto max-w-2xl space-y-3 text-center">
              <p className="text-xs font-bold uppercase tracking-[0.25em] text-primary">Step 1 of 3</p>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Select your level</h2>
              <p className="text-muted-foreground text-sm">Choose your secondary school year to begin</p>
            </div>

            <motion.div
              variants={staggerList} initial="initial" animate="animate"
            ...
            >
              {LEVELS.map((level) => (
                <motion.button
                  ...
                  className={cn(
                    "group relative flex min-h-[22rem] flex-col items-center justify-between overflow-hidden rounded-[2rem] border border-border/60 bg-background/80 p-9 text-center transition-all duration-400",
                    level.ring, "hover:shadow-[0_32px_72px_-36px_rgba(84,56,44,0.34)]"
                  )}
                >
        ...
        {step === "subject" && (
          <motion.div key="subject" {...slideIn} className="space-y-10">
            <div className="mx-auto max-w-2xl space-y-3 text-center">
              <p className="text-xs font-bold uppercase tracking-[0.25em] text-primary">Step 2 of 3</p>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Choose your subject</h2>
              <p className="text-muted-foreground text-sm">Select E Math (compulsory) or A Math (elective)</p>
            </div>
            ...
            >
              {SUBJECTS.map((sub) => (
                <motion.button
                  ...
                  className={cn(
                    "group relative flex min-h-[22rem] flex-col items-center justify-between overflow-hidden rounded-[2rem] border border-border/60 bg-background/80 p-10 text-center transition-all duration-400",
                    sub.ring, "hover:shadow-[0_32px_72px_-36px_rgba(84,56,44,0.34)]"
                  )}
                >
        ...
        {step === "chapter" && (
          <motion.div key="chapter" {...slideIn} className="space-y-8">
            <div className="mx-auto max-w-2xl space-y-2 text-center">
              <p className="text-xs font-bold uppercase tracking-[0.25em] text-primary">Step {selectedSubject ? "3" : "2"} of {selectedSubject ? "3" : "2"}</p>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Select a chapter</h2>
            </div>
            ...
            {chaptersLoading ? (
              ...
              >
                {Object.entries(chaptersByStrand).map(([strand, strandChapters]) => {
                  const meta = STRAND_META[strand] || STRAND_FALLBACK;
                      ...
                      <div className="space-y-3">
                        {strandChapters.map((chapter, idx) => (
                          <motion.button
                            ...
                            className={cn(
                              "group relative flex w-full items-center gap-5 overflow-hidden rounded-[1.75rem] border border-border/60 bg-background/80 px-6 py-5 text-left transition-all duration-300",
                              "hover:border-primary/45 hover:shadow-[0_20px_48px_-28px_rgba(84,56,44,0.28)]"
                            )}
                          >
                            {/* Sweep animation */}
                            ...
                            <span className={cn(
                              "relative z-10 w-11 h-11 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all duration-300",
                              meta.bg, meta.text, "group-hover:bg-primary group-hover:text-primary-foreground"
                            )}>
                              {chapter.code}
                            </span>

                            <div className="relative z-10 flex-1 min-w-0">
                              <span className="font-bold text-base block leading-snug">{chapter.title}</span>
                              {chapter.desc && (
                                <span className="text-sm text-muted-foreground mt-1 line-clamp-1">{chapter.desc}</span>
                              )}
                            </div>
        ...
        {step === "content" && selectedChapter && (
          ...
          >
            ...
            >
              {(() => {
                const meta = STRAND_META[selectedChapter.strand] || STRAND_FALLBACK;
                const StrandIcon = meta.icon;
                return (
                  <>
            ...
            {contentLoading ? (
              ...
              >
                ...
                <motion.div variants={listItem} className="space-y-4">
                  ...
                  {notes.length === 0 ? (
                    <EmptyState icon={FileText} message="Notes for this chapter haven't been uploaded yet." />
                  ) : (
                    <div className="space-y-3">
                      {notes.map((note, idx) => (
                        <motion.a
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-base">{note.title}</p>
                            {note.description && <p className="text-sm text-muted-foreground mt-1 truncate">{note.description}</p>}
                          </div>
                          <div className="flex items-center gap-2 text-sm font-bold text-muted-foreground group-hover:text-primary transition-colors shrink-0">
                            <span className="hidden sm:block">Open PDF</span>
                ...
                <motion.div variants={listItem} className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[oklch(0.82_0.08_55)/40] flex items-center justify-center">
                      <PenTool className="w-5 h-5 text-[oklch(0.40_0.10_55)]" />
                    </div>
                    <span className="font-bold text-base">Practice</span>
                    {practiceItems.length > 0 && (
                      <motion.span
                        initial={{ scale: 0 }} animate={{ scale: 1 }}
                        className="text-xs font-bold bg-[oklch(0.82_0.08_55)/40] text-[oklch(0.35_0.09_55)] px-2.5 py-1 rounded-full"
                      >
                  ...
                  {practiceItems.length === 0 ? (
                    <EmptyState icon={PenTool} message="Practice papers haven't been uploaded yet." />
                  ) : (
                    <div className="space-y-3">
                      {practiceItems.map((item, idx) => (
                        <motion.a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="group flex items-center gap-5 rounded-[1.75rem] border border-border/60 bg-background/80 px-6 py-5 transition-all duration-250 hover:border-[oklch(0.62_0.12_55)/50] hover:bg-[oklch(0.82_0.08_55)/10] hover:shadow-[0_18px_42px_-26px_rgba(84,56,44,0.26)]"
                        >
                          <div className="w-11 h-11 rounded-xl bg-[oklch(0.82_0.08_55)/40] flex items-center justify-center shrink-0 group-hover:bg-[oklch(0.62_0.12_55)] transition-colors duration-250">
                            <span className="text-sm font-bold text-[oklch(0.40_0.10_55)] group-hover:text-white transition-colors">{idx + 1}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-base">{item.title}</p>
                            {item.description && <p className="text-sm text-muted-foreground mt-1 truncate">{item.description}</p>}
                          </div>
                          <div className="flex items-center gap-2 text-sm font-bold text-muted-foreground group-hover:text-[oklch(0.40_0.10_55)] transition-colors shrink-0">
...
function EmptyState({ icon: Icon, message }: { icon: React.ElementType; message: string }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-2 rounded-[1.5rem] border border-dashed border-border/55 bg-muted/12 py-12 text-center"
    >
      <Icon className="w-7 h-7 mx-auto text-muted-foreground/25" />
      <p className="text-sm text-muted-foreground">{message}</p>
      <p className="text-xs text-muted-foreground/50">Check back soon.</p>
    </motion.div>
  );
}
