...
const testimonials = [
  {
    quote: "My daughter used to dread Maths. After three months at Satori Studio, she actually looks forward to it. The difference is real.",
...
const LEAF_PATH = "M34.5 7.8 C34 5.8, 36.5 4.5, 38 6.5 C36.5 7.8, 35 8.8, 34.5 7.8Z";

// 3 rings — each a distinct Satori brand colour, looping forever
const RINGS = [
  {
    // Terracotta — largest, centred, slow drift
    size: "min(72vw, 72vh)",
    left: "50%", top: "50%", x: "-50%", y: "-50%",
    rot: -6,
    stroke: "#C2705A",   // terracotta
    glow: "rgba(194,112,90,0.13)",
    sw: 2.2,
    drawDur: 3.0,
    drawDelay: 0.1,
    loopDur: 14,
  ...
  {
    // Sage green — mid-left top
    size: "min(44vw, 44vh)",
    left: "14%", top: "22%", x: "-50%", y: "-50%",
    rot: 18,
    stroke: "#7FAF88",   // muted sage
    glow: "rgba(127,175,136,0.12)",
    sw: 2.6,
    drawDur: 2.5,
    drawDelay: 0.55,
    loopDur: 11,
  ...
  {
    // Dusty gold — right side
    size: "min(40vw, 40vh)",
    left: "86%", top: "68%", x: "-50%", y: "-50%",
    rot: -28,
    stroke: "#C4984A",   // warm dusty gold
    glow: "rgba(196,152,74,0.11)",
    sw: 2.8,
    drawDur: 2.3,
    drawDelay: 1.05,
    loopDur: 12,
    ...
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden>
      {RINGS.map((ring, i) => (
        <motion.div
          ...
          // Phase 1: draw in on load, then loop gently forever
          initial={{ opacity: 0, scale: 0.94 }}
          animate={{
            opacity: [0, ring.peakOpacity, ring.peakOpacity, ring.restOpacity],
            scale: [0.94, 1.01, 1, 1],
            rotate: [ring.rot, ring.rot + 3, ring.rot - 3, ring.rot],
          }}
          transition={{
            opacity: {
              duration: ring.drawDur + 1.4,
              times: [0, 0.09, 0.72, 1],
              ease: "easeInOut",
              delay: ring.drawDelay,
            },
            scale: {
              duration: ring.drawDur + 1.4,
              times: [0, 0.09, 0.72, 1],
            ...
            rotate: {
              duration: ring.loopDur,
              delay: ring.drawDelay + ring.loopDelay + ring.drawDur,
              repeat: Infinity,
              repeatType: "mirror",
              ease: "easeInOut",
            },
        ...
        >
          <svg
            ...
            style={{ filter: `drop-shadow(0 16px 32px ${ring.glow})` }}
          >
            {/* Draw-on animation */}
            <motion.path
              d={ENSO_PATH}
              ...
              transition={{
                duration: ring.drawDur,
                ease: [0.65, 0, 0.25, 1],
                delay: ring.drawDelay,
              }}
            />
            {/* Continuous slow pulse along the stroke after draw */}
            <motion.path
              d={ENSO_PATH}
              pathLength={1}
              stroke={ring.stroke}
              strokeWidth={ring.sw * 0.5}
              strokeLinecap="round"
              fill="none"
              strokeDasharray="0.08 0.92"
              initial={{ strokeDashoffset: 0, opacity: 0 }}
              animate={{
                strokeDashoffset: [-0.08, -1],
                opacity: [0, 0.6, 0.6, 0],
              }}
              transition={{
                strokeDashoffset: {
                  duration: ring.loopDur * 0.9,
                  delay: ring.drawDelay + ring.drawDur + 0.4,
                  repeat: Infinity,
                  ease: "linear",
                },
                opacity: {
            ...
            <motion.path
              ...
              animate={{ opacity: i === 0 ? 0.9 : 0.7, scale: 1, rotate: 0 }}
              transition={{
                duration: 0.4,
                delay: ring.drawDelay + ring.drawDur - 0.1,
                ease: "backOut",
              }}
      ...
      {/* Warm central glow */}
      <motion.div
        className="absolute left-1/2 top-1/2 h-[56vmin] w-[56vmin] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/10 blur-[88px]"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: [0, 0.6, 0.25], scale: [0.8, 1.05, 1] }}
      ...
      <footer className="border-t border-border/40 px-5 py-10">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-muted-foreground">
            ...
            <span className="text-sm font-semibold">Satori Studio</span>
          </div>
          <p className="text-xs text-muted-foreground text-center">
            © {new Date().getFullYear()} Satori Studio · Boutique Learning Space · Singapore
          </p>
